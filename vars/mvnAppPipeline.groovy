def call(Map options = [:]) {
  ...
  node('BUILD') {
    ...
    def releaseVersion = ''
    def channel = options.slackChannel //'#jaf-brazil-ci'
    def image = options.image // 'maven:3.5-jdk-8' {"image": "'maven:3.5-jdk-8", "channel": "#jaf-brazil-ci"}
      ...
      withDockerContainer(args: '--privileged -u 0 -v /data/workspace/.m2/:/root/.m2/' , image: image) {
        ...
        if (isMasterBranch() || isJava8Branch()) {
          ...
          stage('S3 CodeDeploy Artifact') {
            ...
            withAWS(credentials:'7742c56f-56a1-4596-a39f-458c44158474') {
              s3Upload(file:"${ARTIFACT}.tgz", bucket:'jaf-deploy', path:"${APPNAME}/", contentEncoding: 'gzip')  
            }
          }
...
def notifyBuild(String buildStatus, String release, String channel) {
  if (channel == null) channel = "#jaf-brazil-ci";
